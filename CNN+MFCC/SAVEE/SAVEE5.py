...
for index, y in enumerate(mylist):
    if mylist[index][0:1] in ['n', 'h', 'a', 'f'] or mylist[index][0:2] in ['sa']:
        # Load
        X, sample_rate = librosa.load('D:\\Third year project\\Dataset\\SAVEE\\' + y, res_type='kaiser_fast', duration=2.5, sr=22050*2, offset=0.5)
...
model.add(layers.Conv1D(256, 5, padding='same', input_shape=(X_train.shape[1], 1), kernel_regularizer=regularizers.l2(0.001)))
...
cnnhistory = model.fit(x_traincnn, y_train, batch_size=16, epochs=1000, validation_data=(x_validationcnn, y_validation))
...
def generate_all_plots(history, label_true, label_pred, label_name, save_path=None, dpi=300):
    # Creating a subgraph layout
    fig, axes = plt.subplots(2, 2, figsize=(20, 16))  # increase image size
...
# Call the integration function to generate the image
generate_all_plots(
    history=cnnhistory,
    label_true=actual_labels,
    label_pred=predicted_labels,
    label_name=emotion_labels,
    save_path="Performance_Plots.png",  # Save the file name, can be changed to None to only display the image
    dpi=300
)

###Radar
# Assuming a classification report has been generated
