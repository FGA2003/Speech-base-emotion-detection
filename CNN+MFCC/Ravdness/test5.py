...
def augment_audio(audio, sr):
    ...
    # 增强方法名称
    augment_methods = ["original", "time_stretch", "pitch_shift", "add_noise", "time_shift", "reverse", "freq_mask"]

    # 原始音频
    augmented_data.append((audio, "original"))

    # 时间拉伸
    if np.random.rand() > 0.5:  # 随机应用
        stretched = librosa.effects.time_stretch(audio, rate=np.random.uniform(0.8, 1.2))
        augmented_data.append((stretched, "time_stretch"))

    # 音高偏移
    if np.random.rand() > 0.5:
        pitched = librosa.effects.pitch_shift(audio, sr=sr, n_steps=np.random.randint(-5, 6))
        augmented_data.append((pitched, "pitch_shift"))

    # 加入噪声
    if np.random.rand() > 0.5:
        noise = np.random.normal(0, 0.005, audio.shape)
        noisy_audio = audio + noise
        augmented_data.append((noisy_audio, "add_noise"))

    # 时间平移
    if np.random.rand() > 0.5:
        shifted = np.roll(audio, np.random.randint(-1000, 1000))
        augmented_data.append((shifted, "time_shift"))

    # 反转音频
    if np.random.rand() > 0.5:
        reversed_audio = audio[::-1]
        augmented_data.append((reversed_audio, "reverse"))

    # 频率掩码
    if np.random.rand() > 0.5:
        mel_spec = librosa.feature.melspectrogram(y=audio, sr=sr, n_mels=128)
        freq_masked = mel_spec.copy()
        freq_masked[np.random.randint(0, freq_masked.shape[0]), :] = 0
        audio_reconstructed = librosa.feature.inverse.mel_to_audio(freq_masked, sr=sr)
        augmented_data.append((audio_reconstructed, "freq_mask"))
...
# Load file list
mylist = os.listdir('D:\Third year project\Dataset\RAVDESS MIX/')
...
for index, y in enumerate(mylist):
    if mylist[index][6:8] not in ['01', '07', '08']:
        # Load audio file
        X, sample_rate = librosa.load(
            'D:\\Third year project\\Dataset\\RAVDESS MIX\\' + y, 
        ...
        # Compute the mean of MFCCs along time axis
...
# Split
X_train, X_temp, y_train, y_temp = train_test_split(features, labels, test_size=2/16, random_state=42, stratify=labels)
...
for i in range(len(X_train)):
    # 获取当前音频和标签
    audio = X_train.iloc[i].to_numpy()  # 获取训练音频
    emotion_label = y_train.iloc[i]  # 获取对应标签
    
    # 对音频进行增强
    augmented_audios = augment_audio(audio, sr=22050)  # 调用数据增强函数

    for aug_audio, method in augmented_audios:
        # 特征提取
        mfccs = librosa.feature.mfcc(y=aug_audio, sr=22050, n_mfcc=13)
        mel_spectrogram = librosa.feature.melspectrogram(y=aug_audio, sr=22050, n_mels=128)
...
# 合并增强后的数据与原始训练数据
X_train_augmented = pd.concat([pd.DataFrame(X_train), pd.DataFrame(augmented_features)], ignore_index=True)
y_train_augmented = pd.concat([pd.DataFrame(y_train), pd.DataFrame(augmented_labels)], ignore_index=True)

# 打乱数据
X_train_augmented, y_train_augmented = shuffle(X_train_augmented, y_train_augmented, random_state=42)

# 打印增强后数据集信息
print(f"Training set size after augmentation: {y_train_augmented.shape[0]}")
...
lb = LabelEncoder()
y_train_encoded = to_categorical(lb.fit_transform(y_train_augmented.values.ravel()))
y_validation_encoded = to_categorical(lb.transform(y_validation.values.ravel()))
y_test_encoded = to_categorical(lb.transform(y_test.values.ravel()))

# Expand dimensions for CNN input
x_traincnn = np.expand_dims(X_train_augmented.to_numpy(), axis=2)
...
model.add(layers.Conv1D(256, 7, padding='same', input_shape=(X_train.shape[1], 1), kernel_regularizer=regularizers.l2(0.001)))
model.add(layers.Activation('relu'))
model.add(layers.BatchNormalization())
# Second convolution layer with L2 regularization + activation + batch normalization
model.add(layers.Conv1D(128, 7, padding='same', kernel_regularizer=regularizers.l2(0.001)))
model.add(layers.Activation('relu'))
model.add(layers.BatchNormalization())
